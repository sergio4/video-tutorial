...
| File | Cosa fa |
|---|---|
| `timeline.json` | **Unica fonte dei tempi**: cue delle azioni e frasi della voce. La leggono immagini e audio |
...
| `audio/make_vo.py` | Voce narrante provvisoria con sintesi Kokoro (voce italiana «Nicola»), controlla le sovrapposizioni |
...
python audio/make_vo.py        # solo se cambiano le frasi: richiede kokoro, soundfile ed espeak-ng
python audio/build_audio.py    # musica + effetti + mix → audio/mix.wav (circa 15 s)
```
Le frasi già generate sono in `audio/vo/`, quindi `build_audio.py` basta per cambiare musica, effetti o tempi. Requisiti: `pip install numpy scipy soundfile`.
...
- **Voce:** sintesi vocale (Kokoro, Apache 2.0), con grafie fonetiche per myFITP, eSports e ID. Da sostituire con un doppiatore.
